...
export default function TextCompletionGame() {
  ...
  const initializeGame = async () => {
    ...
    const level2Time = parseInt(localStorage.getItem('level2Time') || '300');
    
    // Set time limit to 80% of level 2 time (20% less)
    const calculatedTimeLimit = Math.max(Math.floor(level2Time * 0.8), 30);
  ...
  const checkAllWords = () => {
    ...
    if (allCorrect) {
      ...
      setIsComplete(true);
      // Store completion time for level 4
      const timeInSeconds = Math.floor((completionTime - startTime) / 1000);
      localStorage.setItem('level3Time', timeInSeconds.toString());
      
      // Auto-redirect to level 4 after 2 seconds
      setTimeout(() => {
        window.location.href = '/game/level4';
      }, 2000);
    }
  };
    ...
    <div className="p-4 max-w-4xl mx-auto min-h-screen bg-white dark:bg-gray-900">
      <div className="flex flex-col items-end gap-4 mb-8">
        <div className="flex items-center justify-between w-full">
          ...
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white">3 שלב - השלמת מילים בזמן מוגבל</h1>
        </div>
